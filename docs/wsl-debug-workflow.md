...
4. **后台启动 Vite 开发服务器**：监听 `http://127.0.0.1:5283`
...
3. **窗口弹出来但白屏**
   - 检查 Vite 那边是不是真起来了（终端有没有 `Local: http://localhost:5283`）
   - 浏览器（Edge 或 Chrome）打开 `http://localhost:5283` 看能不能直接访问，不能的话是前端的问题，跟 Tauri 无关
