## Agent 沟通偏好
- 与用户交互时默认使用中文回复。
- 避免大量术语；解释技术点时尽量通俗，不假设用户具备很深的背景知识。
- 在此仓库中创建 git commit 时，默认使用中文提交信息。

## 仓库工作流

- 本仓库是桌面优先产品。默认应把 Tauri 桌面应用视为主要运行目标，而不是独立浏览器产品。
- 本地开发默认优先使用 `bun run dev` 作为入口；这是本仓库日常开发的推荐工作流。
- 除非用户明确要求只做浏览器场景工作，否则不要把 `web/` 当作独立发布的浏览器应用来处理。
- 涉及跨层改动时，要保持当前架构边界：`web/` 负责桌面前端，`src-tauri/` 负责 Tauri 桌面壳，`agent-sidecar/` 负责 Node sidecar，`scripts/` 负责开发和构建编排。
- 改动应落在正确层级，不要因为前端改起来更快，就把 sidecar 或桌面壳的职责偷放到前端里。

## 验证要求

- 完成代码修改后，应执行与改动范围匹配、且尽可能小而有效的验证，并在汇报中说明验证了什么。
- 修改 `web/` 时，至少优先做一次前端构建或等价的定向验证。
- 修改 `agent-sidecar/` 时，至少优先做一次 sidecar 构建或等价的定向验证。
- 修改涉及桌面集成、运行时链路或 Tauri 配置时，优先验证桌面启动链路，而不是只检查孤立模块。
- 如果改动了附近本来就有测试的文件，除非用户明确要求不跑，或者环境确实不允许，否则应运行相关测试。
- 不要留下只为排查问题临时创建的一次性测试文件。结束前应删除临时测试，或把它们明确整理成现有测试体系里的回归测试，并在汇报中说明原因。
- 如果没法完成有意义的验证，要明确说明缺口，而不是含糊带过。

## 生成产物

- 构建输出和生成目录都应视为派生产物，不是主要编辑目标。
- 除非用户明确要求处理生成产物，否则不要手改 `web/dist`、`agent-sidecar/dist`、`src-tauri/target`、`node_modules` 或 `test-results`。
- 应优先修改源码，再通过正常构建流程重新生成产物，而不是直接 patch 编译结果。

## UI 设计规范遵循

- 所有 UI 相关工作都必须严格遵循 `DESIGN.md` 中定义的规则。
- 所有前端相关工作也必须严格遵循 `web-design-guidelines` 中的规则；这些规则属于强制要求，不是可选建议。
- `frontend-design` 主要负责仓库内的生产级前端实现，尤其适合把已经确认的设计方向落成 React 组件、页面和可交付 UI 代码。
- 对于 React 前端实现、代码审查、重构和性能优化相关工作，默认应将 `vercel-react-best-practices` 作为性能层面的主要参考。
- 尤其是在修改组件结构、渲染行为、状态订阅、异步数据链路或对 bundle 体积敏感的前端代码时，应优先应用 `vercel-react-best-practices`。
- 在设计、实现或审查 UI 时，应将 `DESIGN.md` 视为视觉风格、布局、组件、间距和交互细节的首要依据。
- 对于前端设计、实现、审查和重构，必须同时满足 `DESIGN.md` 与 `web-design-guidelines` 两套要求。
- 如果现有 UI 实现与 `DESIGN.md` 冲突，除非用户明确要求例外，否则应优先使结果与 `DESIGN.md` 保持一致。

## 复杂问题排查

- 分析任何问题时，都要优先做深入分析，先找出问题的根本原因，再提出或实施修复；如果底层原因还不清楚，不要停留在表面现象上。
- 查资料时统一优先使用 Context7 获取框架或库的官方文档，使用 GitHub Issues / Discussions / PR 查真实案例与修复思路；当现象反直觉时，不要只凭本地经验判断。
- 输出诊断结论时，要明确区分：已经确认的事实、基于证据的推断，以及作为启发来源的外部案例。

## 运行时与安全边界

- 对前端代码来说，在可行情况下要保留浏览器开发态兼容性；不要无保护地假设 Tauri 专属全局变量或桌面端 API 一定存在。
- 修改 Tauri capability、CSP、桌面运行时链路或命令审批逻辑时，要特别谨慎。
- 除非任务明确要求，否则不要放宽安全敏感权限、自动批准行为或网络访问规则。
- 如果任务确实要求修改这些安全敏感区域，要明确说明风险，并做更谨慎的验证。

## 测试清理

- 涉及浏览器的测试默认一律使用 headless 模式运行，除非用户明确要求使用有界面模式。
- 在开始浏览器自动化前，如需后端或 dev server，必须先检查目标服务是否已经在后台运行；如果已经启动，优先复用现有服务。
- 做浏览器自动测试时，不要擅自再启动后台服务；如果用户已经启动了服务，应避免重复启动和端口冲突。
- 开始浏览器测试时，不要让后端服务以前台阻塞方式运行；如需启动后端，必须放到后台或使用脱离终端的方式，避免测试流程被卡住。
- 测试完成后，清理本次运行产生的缓存文件、临时产物和测试输出，不要把垃圾文件留在仓库里。
- 新建的临时 `*.test.*`、`*.spec.*`、fixture 和复现文件默认都视为临时产物，除非它们是有意纳入版本控制的回归测试；临时文件必须在最终回复前删除。
- 浏览器测试结束后，清理本次启动但未退出的后端、浏览器、驱动或相关测试进程，避免残余进程累积导致电脑变卡甚至卡死。

## 文档同步

- 修改仓库规则或面向用户的项目文档时，除非用户明确要求只改单语版本，否则应同步保持中英文版本一致。
- 对于 `AGENTS.md` / `docs/AGENTS.zh-CN.md`、`README.md` / `docs/README.zh-CN.md`、`DESIGN.md` / `docs/DESIGN.zh-CN.md` 这类成对文件，应追求语义一致，而不是逐行直译。
